...
interface DBIngredientDetailProps {
  item: DBIngredient;
  onBack: () => void;
  onItemUpdated: (item: DBIngredient) => void;
}

const API_BASE = process.env.NEXT_PUBLIC_API_URL ?? "http://localhost:8000";

const MACRO_OPTIONS = ["carbohydrate", "fat", "protein", "water", "fiber", "none"];
const ALLERGY_TYPES = ["milk", "egg", "peanut", "gluten", "tree_nut", "shellfish", "sesame"];
...
const FUNCTIONAL_ROLE_DB_OPTIONS = [
  "emulsifier", "stabilizer", "preservative", "acidity_regulator",
  "anti_caking_agent", "raising_agent", "thickener", "colorant",
  "flavoring_agent", "flavor_enhancer", "fat_source", "sweetener",
  "structure_provider", "fruit_component", "protein_source",
  "humectant", "bleaching_agent", "bulking_agent",
];
const WATCHLIST_CATEGORY_OPTIONS = [
  "high_sugar", "added_sugar", "artificial_color", "artificial_flavor",
  "natural_flavor", "artificial_sweetener", "processed_fat",
  "high_sodium", "highly_processed", "refined_carbohydrate",
  "caffeine", "banned_ingredient",
];

export default function DBIngredientDetail({ item, onBack, onItemUpdated }: DBIngredientDetailProps) {
  ...
  async function handleSave() {
    ...
    try {
      const response = await fetch(`${API_BASE}/api/sme/db-ingredients/${item.id}`, {
        method: "PATCH",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({
          name,
          json_data: { ...jsonData, common_names: name },
        }),
      });
      if (!response.ok) {
      ...
      setSuccessMessage("Saved to database");
  ...
  return (
    <div className="space-y-6 pb-8">
      ...
      <div className="flex items-center gap-3">
        ...
        <div>
          <h2 className="text-2xl font-bold">Edit DB Ingredient</h2>
          <p className="text-xs text-slate-500 mt-1">ingredonly ID: {item.id}</p>
        </div>
      </div>
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
        ...
        <div>
          <label className="block text-xs font-medium text-slate-700 mb-2">
            Name (common names, comma-separated)
          </label>
          <input
            type="text"
            value={name}
      ...
      <div className="bg-white rounded-2xl shadow-sm border border-slate-100 p-6 space-y-4">
        ...
        {["primary_macro", "secondary_macro", "tertiary_macro"].map((macroKey) => (
          <div key={macroKey}>
            <label className="block text-xs font-medium text-slate-700 mb-2">
              {macroKey.split("_").map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join(" ")}
            </label>
            <select
              value={jsonData.macro_profile?.[macroKey] || ""}
              onChange={(e) =>
                handleNestedChange("macro_profile", macroKey, e.target.value === "none" ? null : e.target.value)
              }
              className="w-full px-3 py-2 border border-slate-200 rounded-lg focus:border-orange-500 focus:outline-none"
            >
              <option value="">Select...</option>
